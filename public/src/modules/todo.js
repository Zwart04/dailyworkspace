(() => {
  ...
  const renderTodo = (view) => {
    ...
    document.getElementById('todoForm').addEventListener('submit', (e) => {
      e.preventDefault();
      const todos = storage.get('todos') || [];
      todos.push({ id: 'T-' + uid().toUpperCase(), text: document.getElementById('t_text').value.trim(), priority: document.getElementById('t_priority').value, done: false, createdAt: nowIso() });
    ...
    document.querySelectorAll('[data-del]').forEach(btn => btn.addEventListener('click', () => {
      const todos = (storage.get('todos') || []).filter(t => t.id !== btn.dataset.del);
      storage.set('todos', todos);
      window.DW.route.navigate();
    }));
  };
